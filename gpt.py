...
def analise(avaliacoes, topicos_positivos, topicos_negativos):
    ...
    for i, avaliacao in enumerate(avaliacoes, start=1):
        ...
        prompt = [
            {"role": "user", "content": f"Analise a avaliação: {avaliacao}. Indique se a avaliação se encaixa em algum dos tópicos positivos ({topicos_positivos}) ou negativos ({topicos_negativos})."}
        ]
...
contagem_positivos, contagem_negativos = analise(avaliacoes, topicos_positivos, topicos_negativos)
